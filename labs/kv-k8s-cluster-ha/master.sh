...
if [ $MASTER_TYPE = "single" ]; then
    ...
    if (( $NODE == 0 )) ; then
        ...
        kubeadm init --control-plane-endpoint "kv-scaler.lab.local:6443" --upload-certs --pod-network-cidr $POD_CIDR --apiserver-cert-extra-sans kv-master.lab.local kv-scaler.lab.local | tee /vagrant/kubeadm-init.out
